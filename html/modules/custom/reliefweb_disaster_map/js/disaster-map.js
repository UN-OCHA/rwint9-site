...
(function () {
  ...
  reliefweb.mapbox.init(drupalSettings.reliefweb.mapbox_key, drupalSettings.reliefweb.mapbox_token);

  // Skip if the browser doesn't support mapbox GL.
  if (!reliefweb || !reliefweb.mapbox || !reliefweb.mapbox.supported()) {
  ...
  function createMap(element) {
    ...
    // Map settings.
    var settings = drupalSettings.reliefweb.maps[element.getAttribute('data-disaster-map')];
    ...
    figure.appendChild(createLegend(settings.legend || {}));
    ...
    button.appendChild(document.createTextNode(settings.close));
