...
/**
 * Contains a form for switching the view mode of a term during preview.
 *
 * @internal
 */
class TermPreviewForm extends FormBase {
  ...
  /**
   * Constructs a new TermPreviewForm.
   *
  ...
  public function getFormId() {
    return 'term_preview_form_select';
  }
   ...
   *   The term being previewed.
   *
   * @return array
   *   The form structure.
   */
  public function buildForm(array $form, FormStateInterface $form_state, EntityInterface $term = NULL) {
    $view_mode = $term->preview_view_mode;

    $query_options = ['query' => ['uuid' => $term->uuid()]];
    ...
    elseif ($query->has('back')) {
      $url = Url::fromUserInput($query->get('back'));
    }
    else {
    ...
    $view_mode_options = [
      'full' => $this->t('Full'),
    ] + $this->entityDisplayRepository->getViewModeOptionsByBundle('taxonomy_term', $term->bundle());
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $route_parameters = [
      'term_preview' => $form_state->getValue('uuid'),
    ...
    $form_state->setRedirect('entity.taxomomy_term.preview', $route_parameters, $options);
  }
