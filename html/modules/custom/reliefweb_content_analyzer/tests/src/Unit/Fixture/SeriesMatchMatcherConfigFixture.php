...
final class SeriesMatchMatcherConfigFixture {
  ...
  public static function defaults(): array {
    return [
      ...
      'ai_title_match_endpoint' => 'http://ocha-ai-helper/text/match/series-title',
      'ai_title_match_min_confidence' => 0.65,
      'ai_title_description_template' => "Extract the title of the document from the source text and rewrite it only enough to match the naming style of these example titles from the same report series. Use the examples for style (separators, language, casing), not for facts. Never invent dates, issue numbers, or week numbers; omit those parts if they are not present in the source text. When grounded markers are listed, copy them exactly or omit them.\n@examples",
      'ai_title_inference' => [
        ...
        'max_tokens' => 512,
        'thinking_mode' => 'none',
        'system_prompt' => 'Generate a title for this humanitarian report that matches the series naming style shown in the structured output schema. Prefer the title as it appears in the document. Never invent dates, issue numbers, or week numbers that are not present in the source text. Use the `structured_output` tool.',
      ],
