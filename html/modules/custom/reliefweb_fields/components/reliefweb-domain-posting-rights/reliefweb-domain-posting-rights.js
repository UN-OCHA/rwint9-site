(function () {
   ...
   */
  ReliefWebDomainPostingRightsField.prototype = {
    ...
    /**
     * Create a filter for domains.
     */
    createDomainFilter: function () {
      let name = 'domain';
      ...
      var span = document.createElement('span');
      span.appendChild(document.createTextNode(t('Domain')));
    ...
    /**
     * Create the domain info row (domain, rights).
     */
    createEntry: function (data) {
      var container = document.createElement('li');
      container.setAttribute('data-domain', data.domain);
      ...
      domain.appendChild(document.createTextNode(data.domain));
      domain.setAttribute('data-label', 'domain');
      info.appendChild(domain);

      // Disable the edit components if the domain is blocked (status == 0).
    ...
    /**
     * Create a new domain edit form for a field.
     */
    createNewEntryForm: function () {
      ...
      input.setAttribute('placeholder', t('Domain'));
    ...
    /**
     * Create the filters to filter the list of domains.
     */
    ...
    /**
     * Check if the given field already contains the domain.
     */
    checkDuplicate: function (field, element, domain) {
      var entries = field.querySelectorAll('li[data-domain]');
      for (var i = 0, l = entries.length; i < l; i++) {
        var entry = entries[i];
        if (entry !== element && entry.getAttribute('data-domain') == domain) {
          return t('The domain already exists in this field.');
        }
      }
      return '';
    },
    ...
    /**
     * Validate a domain, calling a validation endpoint for the field.
     */
    validateData: function (data, field, element, callback) {
      var self = this;
      var settings = this.getFieldSettings();
      // Disable the form while validating the domain data.
      ...
      xhr.addEventListener('load', function () {
        ...
        try {
          ...
          error = data.error || self.checkDuplicate(field, element, data.domain);
        }
    ...
    /**
     * Extract the domain list for a given field.
     */
    parseList: function (field) {
      var entries = this.field.querySelectorAll('li[data-domain]');

      // List of domains.
      var list = [];

      // List of already added domains to avoid duplicates.
      var added = {};
      ...
      for (var i = entries.length - 1; i >= 0; i--) {
        ...
        // a domain.
        if (!added.hasOwnProperty(data.domain)) {
          list.push(data);
          added[data.domain] = true;
        }
      }
    ...
    /**
     * Extract the domain data from the store.
     */
    ...
    handleChange: function (event) {
      ...
      if (target && target.tagName === 'SELECT') {
        ...
        if (name === 'job' || name === 'training' || name === 'report') {
          ...
          // from a domain row and we get the list element.
        ...
        // @todo use "domain" instead of name.
        if (parent.hasAttribute('data-filters') && name === 'domain') {
          let grandParent = parent.parentNode;
          if (grandParent.querySelectorAll('li[data-domain-filtered]').length > 0) {
            Array.from(grandParent.querySelectorAll('li[data-domain-filtered]'))
            .forEach(e => e.removeAttribute('data-domain-filtered'));
          }

          if (target.value !== '') {
            Array.from(grandParent.querySelectorAll('li[data-domain*="' + target.value + '"]'))
            .forEach(e => e.setAttribute('data-domain-filtered', ''));
          }
